  ...
  (insta/transform
   {:verb (partial param-map :verb)
    ...
    :trope (fn [& args] {:trope {:roles (vec (set (concat (walk-get-key :role (first args)) (walk-get-key :role-a (first args)) (walk-get-key :role-b (first args)))))
                                 :objects (vec (set (concat (walk-get-key :object (first args)) (walk-get-key :place (first args)))))
                                 :events (mapcat :events args)
                                 :situations (mapcat :situation args)}})
    ;; MESSY!
    ;; :trope (fn [& args] {:trope {:roles (into [] (set (map str/capitalize (remove #(or (empty? %) (nil? %)) (concat (get-by-key :role (mapcat :events args)) (get-by-key :role (mapcat :situations args)))))))
    ;;                              :objects (into [] (set (map str/capitalize (remove #(or (empty? %) (nil? %)) (concat (get-by-key :object (mapcat :events args)) (get-by-key :object (mapcat :situations args)) (get-by-key :place (mapcat :events args)) (get-by-key :place (mapcat :situations args)))))))
    ;;                              :events (mapcat :events args)
    ;;                              :situations (mapcat :situation args)}})
    }
   ptree))
